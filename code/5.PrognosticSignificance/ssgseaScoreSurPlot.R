...
##############
# tcgaGseaRes <- readRDS(file = '/data/tcgaRhoFamiliesGseaRes.rds')
tcgaSsgseaRes <- readRDS(file = '/data/tcgaRhoFamiliesSsgseaRes.rds')
panCanCliData <- readRDS(file = '/data/tcgaPanCanCliData.rds')


# subtype
# tcgaGseaRes <- tcgaGseaRes %>% 
#   mutate(subtype = ifelse(NES > 0 & pvalue < 0.1, 'Up', ifelse(NES < 0 & pvalue < 0.1, 'Down', 'Neutral')), 
#          patientID = substr(SAMPLE_BARCODE, 1, 12))


SubFunc <- function(data){
  
  data <- data %>% mutate(subtype = ifelse(ssgseaScore > quantile(ssgseaScore , 0.75), 'Up', 
                                           ifelse(ssgseaScore  < quantile(ssgseaScore , 0.25), 'Down', 'Neutral')), 
                          patientID = substr(SAMPLE_BARCODE, 1, 12))
  
  return(data)
}

# tcgaGseaRes <- tcgaGseaRes %>% group_by(DISEASE, ID) %>% do(SubFunc(.))
tcgaSsgseaRes <- tcgaSsgseaRes %>% group_by(DISEASE, ID) %>% do(SubFunc(.))



panCanCliData <- panCanCliData %>% tibble::rownames_to_column(var = "patientID")
panCanCliData <- panCanCliData %>% right_join(tcgaSsgseaRes, by = 'patientID')


osCanTypes <- c('ACC','BLCA','BRCA','CESC','CHOL','COAD','ESCA','GBM','HNSC', 'KIRC','KIRP','LAML','LGG',
                'LIHC','LUAD','LUSC','MESO','OV','PAAD', 'READ','SARC','SKCM','STAD','THCA','UCEC','UCS','UVM')
...
panCanCliData <- split.data.frame(panCanCliData, f = ~ ID)


subtypeSurplots <- lapply(panCanCliData, function(cliData){
  
  cliData <- split.data.frame(cliData, f = ~ DISEASE)
  
  surPlots <- lapply(cliData, function(surData){
    
    title <- paste(surData$DISEASE[1], surData$ID[1], sep = '_')
    
    surData <- surData[, c('patientID', 'os', 'os_time', 'subtype')]
    colnames(surData) <- c('patientID', 'event', 'time', 'sample.label')
    
    surplot <- try(PlotSurv(clinical.data = surData, xscale = 'd_y', xlab = 'Time in years', 
                            ylab = 'Overall survival', risk.table = FALSE, median.time = FALSE, main = title))
...
##############plot  
pdf(file = '/result/Section3/rhoGTPaseSur.pdf')

for(i in seq(length(subtypeSurplots$RhoFamilyGTPases))){
  print(subtypeSurplots$RhoFamilyGTPases[[i]])
}

dev.off()


pdf(file = '/result/Section2/rhoGDISur.pdf')

for(i in seq(length(subtypeSurplots$RhoGDIProteinFamily))){
  print(subtypeSurplots$RhoGDIProteinFamily[[i]])
}

dev.off()


pdf(file = '/result/Section2/rhoGapSur.pdf')

for(i in seq(length(subtypeSurplots$RhoGTPaseActivatingProteins))){
  print(subtypeSurplots$RhoGTPaseActivatingProteins[[i]])
}

dev.off()


pdf(file = '/result/Section2/rhoGEFSur.pdf')

for(i in seq(length(subtypeSurplots$RhoFamilyGTPases))){
  print(subtypeSurplots$RhoGuanineNucleotideExchangeFactors[[i]])
}
