...
selectSurPvalue <- selectSurPvalue %>% group_by(Class, geneSymbol) %>% count() %>% group_by(Class) %>% arrange(desc(n)) %>% slice_head(n = 10) %>%
...
showGenes <- selectSurPvalue %>% group_by(Class, geneSymbol) %>% count() %>% group_by(Class) %>% arrange(desc(n)) %>% slice_head(n = 10)
...
#########################
# panCanEnrichScoreSurPvalue <- readRDS(file = '/data/rhoDiffValida/panCanRhoEnrichScoreSurPvalue.rds')
# 
...
pdf('/result/Section3/validation/Figure4.1.pdf')
plot1
plot2
plot3
# plot4
