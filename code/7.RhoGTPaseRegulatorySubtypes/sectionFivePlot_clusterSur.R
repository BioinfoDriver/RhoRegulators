...
########
setwd('/sesult/Section5/panCanCluster')
panCanCluster <- read.csv(file='panCanCluster.k=6.consensusClass.csv', header = F, sep = ',', stringsAsFactors = FALSE)
panCanCluster <- panCanCluster %>% dplyr::rename(SAMPLE_BARCODE = V1, Clusters = V2)  %>% mutate(patientID = substr(SAMPLE_BARCODE, 1, 12))
...
#########
pdf(file = '/result/Section5/panCanClusterSurplot.pdf')
...
pdf(file = '/result/Section5/eachCanClusterSurplot.pdf')
